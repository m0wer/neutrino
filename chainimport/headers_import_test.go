...
func TestValidatePairWithNonMonotonicTimestamps(t *testing.T) {
	...
	for i := 0; i < numHeaders; i++ {
		...
		// timestamps is at index 7 (median of indices 3..13), which
		// is ~70 minutes after baseTime. The last header at index 14
		// gets timestamp = 130 - 5 = 125 minutes, well after 70 min.
		if i == numHeaders-1 {
			prevTs := baseTime.Add(
	...
	err := validator.ValidatePair(headers[numHeaders-2], headers[numHeaders-1])
